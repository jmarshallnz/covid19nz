...
library(readxl)
library(Manu)
library(DHBins) #<- JM hacked version: https://github.com/jmarshallnz/DHBins/tree/covid_dhbs
...
vacc_dhbs %>%
  ...
  mutate(None = Population - Dose1,
         One = Dose1 - Dose2,
         Two = Dose2) %>%
  select(DHB, Age, None:Two) %>%
  ungroup() %>%
...
ggplot(tris_long) +
  geom_dhbtri(aes(map_id=DHB,class_id=tri_id, fill=Vacc), alpha=0.7)+
