...
dhb_files <- data.frame(files = list.files('data/dhb_daily/maori', '*.csv', full.names = TRUE))
dailies <- dhb_files %>%
...
png("today_maori_dose1.png", width=1800, height=1280)
ggplot(dose1_labelled %>% filter(Today == "Today"),
  ...
  annotate(geom="curve",curvature=0.2,x=0.63,y=15.8,xend=0.637,yend=15,arrow=arrow(angle=20, type='closed'), col="grey70") +
...
#            col = "grey50", vjust=-0.8, size=8) +
  ...
  labs(x = NULL,
       ...
       title = paste("Path to 90%: M\u0101ori first doses to", todays_date),
...
png("today_maori_dose2.png", width=1800, height=1280)
ggplot(dose2 %>% filter(Today == "Today"),
  ...
  geom_vline(xintercept=0.9) +
  geom_point(aes(col=Vacc > 0.9), size=8) +
  annotate(geom="curve",curvature=-0.2,x=0.453,y=17.2,xend=0.465,yend=18,arrow=arrow(angle=20, type='closed'), col="grey70") +
  ...
  scale_fill_manual(values = c(`days,` = 'white', weeks = 'grey70'),
                    guide = guide_legend(override.aes = list(size=5))) +
  theme_minimal(base_size=36, base_family = "ssp") +
  scale_x_continuous(labels = scales::label_percent(), breaks=c(0.5,0.6,0.7,0.8,0.9), expand=c(0,0.005,0,0.015)) +
  theme(panel.grid.major.y = element_line(color='grey96', size=0.5),
        axis.text = element_text(size = rel(0.7)),
        plot.title = element_text(face="bold"),
        plot.subtitle = element_text(size = rel(0.9)),
  ...
  labs(x = NULL,
       ...
       subtitle = paste0("How the ",
                         ...
                         today,
                         " move each DHB towards 90%"),
       title = paste("Path to 90%: M\u0101ori second doses to", todays_date),
       tag = "Data from Ministry of Health. Chart by Jonathan Marshall. https://github.com/jmarshallnz/covid19nz")
dev.off()
