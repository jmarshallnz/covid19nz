...
dhb_files <- data.frame(files = list.files('data/dhb_daily/5-11', '*.csv', full.names = TRUE))
dailies <- dhb_files %>%
  extract(files, into="date", regex="([0-9-]+).csv", remove=FALSE) %>%
...
png("today_5_11_dose1.png", width=1800, height=1280)
